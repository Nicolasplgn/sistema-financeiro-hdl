...
const IntelligenceHub = ({ companyId, apiBase, onNavigate }) => {
  ...
  if (loading) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100 m-6 shadow-2xl">
        <div className="w-16 h-16 border-4 border-slate-100 border-t-blue-600 rounded-full animate-spin"></div>
        <p className="mt-4 font-black text-[9px] uppercase tracking-[0.3em] text-slate-400">Processando Inteligência Vector...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8 pb-20 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        <div>
           <h1 className="text-2xl font-black text-slate-900 tracking-tighter flex items-center gap-3 italic">
             <BrainCircuit className="text-blue-600" size={36}/> Vector Intelligence Hub
           </h1>
           <p className="text-slate-400 font-bold text-sm uppercase tracking-widest text-[9px] mt-1">Status Operacional e Projeções de Mercado</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-4 bg-white p-2 rounded-[2.5rem] border border-slate-200 shadow-xl">
          <button onClick={() => setIsGoalModalOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition shadow-lg active:scale-95">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* CARD SCORE */}
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="lg:col-span-2 bg-slate-900 rounded-2xl p-6 text-white relative overflow-hidden shadow-2xl border border-white/5">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
            <div className="relative flex items-center justify-center">
               ...
               <div className="absolute flex flex-col items-center">
                  <span className="text-4xl font-black font-mono tracking-tighter italic">{financialScore}</span>
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">Vector Score</span>
               </div>
            </div>
            <div className="flex-1 text-center md:text-left space-y-6">
                <h2 className="text-2xl font-black tracking-tighter italic">Saúde Operacional</h2>
                ...
                <div className="flex gap-4 justify-center md:justify-start">
                   <div className="px-5 py-2.5 bg-white/5 rounded-xl border border-white/10 text-emerald-400 text-[9px] font-black uppercase tracking-widest italic">Auditoria OK</div>
                   <div className="px-5 py-2.5 bg-white/5 rounded-xl border border-white/10 text-blue-400 text-[9px] font-black uppercase tracking-widest italic">Dados em Realtime</div>
                </div>
            </div>
          </div>
        </motion.div>
        ...
        {/* CARD INSIGHTS */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-4 flex-1">
            ...
            {!data.insights?.length && <div className="h-40 flex flex-col items-center justify-center text-slate-300 opacity-30"><Lightbulb size={48}/><p className="text-[9px] font-black mt-2">NADA A REPORTAR</p></div>}
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="p-8 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Meta Definida</p>
                <p className="text-sm lg:text-2xl font-black text-slate-900 font-mono tracking-tighter italic truncate">{formatBRL(data.planned)}</p>
              </div>
              <div className="p-8 bg-emerald-50 rounded-2xl border border-emerald-100 text-center">
                <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-2 italic">Realizado {selectedYear}</p>
                <p className="text-sm lg:text-2xl font-black text-emerald-700 font-mono tracking-tighter italic truncate">{formatBRL(totalRealizadoAcumulado)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center justify-center py-6">
             <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">Estimativa Próximo Período</p>
             <div className="flex items-center gap-4 lg:gap-6 w-full justify-center">
                <h4 className="text-2xl lg:text-5xl font-black font-mono italic text-emerald-400 tracking-tighter truncate max-w-full">{formatBRL(data.forecast)}</h4>
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4 mb-12">
           ...
           <div>
              ...
              <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest">Compliance e Rastreabilidade do Ciclo BI</p>
           </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
           {auditCards.map((item, i) => (
             <div 
               ...
               className="group cursor-pointer p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-slate-900 hover:text-white transition-all duration-500 shadow-sm hover:shadow-2xl"
             >
                ...
                <div className="mt-8 flex items-center gap-2 text-blue-600 font-black text-[9px] uppercase tracking-widest group-hover:text-white">Acessar <ArrowRight size={14}/></div>
             </div>
           ))}
        </div>
      </div>
      ...
      <AnimatePresence>
        {isGoalModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-50 flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-slate-100">
               <div className="p-6 text-center space-y-8">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 font-black text-[9px] uppercase tracking-widest mt-2">Ano Competência: {selectedYear}</p>
                  </div>
                  <div className="flex items-center justify-center bg-slate-50 border-2 border-slate-200 rounded-[2rem] px-6 py-8 focus-within:border-blue-600 focus-within:ring-4 focus-within:ring-blue-600/10 transition-all">
                    ...
                    <input type="text" value={goalValue} onChange={handleCurrencyChange} placeholder="0,00" className="bg-transparent border-none outline-none text-2xl lg:text-5xl font-black text-slate-900 w-full text-center placeholder-slate-200 font-mono italic tracking-tighter z-10"/>
